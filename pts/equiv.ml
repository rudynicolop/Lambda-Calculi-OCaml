...
(** Weak-head reduction. *)
let rec weak_red : 's. 's b_term -> 's b_term option = function
...
let rec weak_norm : 's. 's b_term -> 's b_term =
  fun t ->
  match weak_red t with
  | Some t' -> weak_norm t'
  | None -> t
    
module EquivWH (SAR : Triple) = struct
  include SAR
  
  let rec (==) (t1 : sort b_term) (t2 : sort b_term) : bool =
    equiv_wf (weak_norm t1) $ weak_norm t2
  
  and equiv_wf (t1 : sort b_term) (t2 : sort b_term) : bool =
    match t1, t2 with
